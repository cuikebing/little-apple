...
+ (CGSize)sizeForRotatedImage:(UIImage *)imageToRotate rotation:(NSInteger)rotation
{
    ...
    CGSize absoluteOutputSize = CGSizeMake(abs(outputSize.width), fabs(outputSize.height));

    return absoluteOutputSize;
}

+ (CGSize)sizeAfterRotate:(CGSize)srcSize rotation:(NSInteger)rotation {
    CGFloat rotationAngle = rotation * M_PI / 2;
    // Image size after the transformation
    CGSize outputSize = CGSizeApplyAffineTransform(srcSize, CGAffineTransformMakeRotation(rotationAngle)); 
    CGSize absoluteOutputSize = CGSizeMake(abs(outputSize.width), fabs(outputSize.height));    
    return absoluteOutputSize;
}
...
+ (UIImage *)rotatedImage:(UIImage *)imageToRotate rotation:(NSInteger)rotation
{
    ...
    CGSize imageSize = imageToRotate.size;
    // Image size after the transformation
    CGSize absoluteOutputSize = [self sizeForRotatedImage:imageToRotate rotation:rotation];
