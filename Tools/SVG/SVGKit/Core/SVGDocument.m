...
- (id)initWithDocumentNamed:(NSString *)documentName andParser:(SVGParser *)parser
{
    ...
    if( self != nil )
    {
        NSAutoreleasePool *pool = [NSAutoreleasePool new];// modify by ckb 2012
...
- (BOOL)parseFileAtPath:(NSString *)aPath error:(NSError **)error
{
    ...
    NSAutoreleasePool *pool = [NSAutoreleasePool new];// modify by ckb 2012
    //@autoreleasepool
    { //stich: hitting really high peak memory when parsing lots of SVGS in dispatch_queue, trying to avoid
...
//! add by ckb。 源代码没有解析宽，高的单位
#define DPI 132
#define MMPERINCH 25.9

-(CGFloat)unitValueToPixel:(NSString*)value{
    if (value == nil) {
        return 0;
    }
    ...
    if([value rangeOfString:cm].length > 0){
        fValue = fValue*10/MMPERINCH*DPI;
    }else if([value rangeOfString:px].length > 0){
        fValue = fValue;
    }else if([value rangeOfString:mm].length > 0){
        fValue = fValue/MMPERINCH*DPI;
    }else if([value rangeOfString:inch].length > 0){
        fValue = fValue*DPI;
    }if([value rangeOfString:percent].length > 0){
...
- (void)parseAttributes:(NSDictionary *)attributes {
	...
	if ((value = [attributes objectForKey:@"width"])) {
        ...
        _width = [self unitValueToPixel:(NSString*)value];// modify by ckb
	}
	
	if ((value = [attributes objectForKey:@"height"])) {
		//_height = [value floatValue];
        _height = [self unitValueToPixel:(NSString*)value];// modify by ckb
	}
