...
@synthesize DashArray = _DashArray;
...
- (void)dealloc {
	//[self loadPath:nil];// modify by ckb
    [_DashArray release];//modify by lww
...
{
    if (_DashArray == nil) {
        _DashArray = [[NSMutableArray alloc] init];
    }

    [_DashArray addObject:f1];
    [_DashArray addObject:f2];
...
- (CALayer *) autoreleasedLayer {
    ...
    if ([_DashArray count] != 0) {//modify my lww
        int a = [(NSString*)[_DashArray objectAtIndex:0] integerValue];
        int b = [(NSString*)[_DashArray objectAtIndex:1] integerValue];
        _shapeLayer.lineDashPattern = [NSArray arrayWithObjects:
