...
- (void)updateCustomString:(NSString *)newText ; //Add by wy 
...
- (void)setCustomString:(NSString *)newText {
	if ([NSThread isMainThread]) {
		[self updateCustomString:newText];
		[self setNeedsLayout];
		[self setNeedsDisplay];
	} else {
		[self performSelectorOnMainThread:@selector(updateCustomString:) withObject:newText waitUntilDone:NO];
...
- (void)updateCustomString:(NSString *)newText {
    if (customString != newText) {
        [customString release];
        customString = [newText copy];
    }
}
...
- (void)updateIndicators {
	...
	//Add by wy 
	else if (mode == MBProgressHUDModeNSString && self.customString ){
		UILabel *stringView = [[UILabel alloc] init];
		
		stringView.font = [UIFont systemFontOfSize:LABELDETAILSFONTSIZE];
		CGSize theSize = [self.customString sizeWithFont:stringView.font];
		stringView.frame = CGRectMake(0, 0,theSize.width  , theSize.height);
		stringView.backgroundColor = [UIColor clearColor];
		stringView.textColor = [UIColor whiteColor];
		stringView.adjustsFontSizeToFitWidth = NO;
		stringView.textAlignment = UITextAlignmentCenter;
		stringView.opaque = NO;
		stringView.text = self.customString;
		//self.customView = stringView;
		self.indicator =stringView;
		[stringView release];
	} else {
...
- (void)layoutSubviews {
	...
	if ([indicator isKindOfClass:[UILabel class]]) {
		indFrame = indicator.frame;
	}else {
